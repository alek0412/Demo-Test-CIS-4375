...
(function () {
  ...
  var altInput = document.getElementById('popular-times-alt');
  ...
  function renderPreviewFromServer(d) {
    ...
    if (d.kind === 'image') {
      ...
      im.src = url;
      im.alt = (altInput && altInput.value) || 'Availability times preview';
  ...
  function renderLocalPreview(file) {
    ...
    if (file.type.indexOf('image/') === 0) {
      ...
      reader.onload = function () {
        ...
        im.alt = (altInput && altInput.value) || '';
        im.className = 'admin-marketing-preview-img';
        preview.appendChild(im);
      };
  ...
  if (altInput) {
    altInput.addEventListener('input', function () {
      var im = preview && preview.querySelector('img.admin-marketing-preview-img');
      if (im) im.alt = altInput.value || '';
    });
  }
